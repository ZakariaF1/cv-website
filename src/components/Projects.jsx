...
function ProjectCard({ project: p }) {
  ...
  const previewSrc = p.screenshots?.[0]

  return (
    <div className={`project-card${p.latest ? ' has-banner' : ''}`}>
      <div className="card-glow" />
      {p.latest && <div className="card-latest">Just Shipped</div>}

      {(p.video || previewSrc) && (
        <div className="card-hero" onClick={() => setLightboxIndex(previewSrc ? (p.video ? 1 : 0) : 0)}>
          {previewSrc && <img src={previewSrc} alt="Preview" className="card-hero-img" loading="lazy" />}
      ...
      {lightboxIndex !== null && (
        <Lightbox project={p} startIndex={lightboxIndex} onClose={() => setLightboxIndex(null)} />
      )}
    </div>
  )
}
